...
if ( ! function_exists('c_format')){
	...
	function url_to_clean($url){
        //$url = 'http://wptest.natanphp.com/blog/?af_id=VHArbWFoUUJ5czZrNFJxeGt4OXpDZz09';
	...
	function wallet_type($value){
		...
		if($value['dis_type'] == 'integration_sale'){
	...
	function parseMessage($text,$data,$control = 'admincontrol', $seprate = false){
		...
		if($control == 'usercontrol'){
			$text = str_replace('external_order_id', ' <a target="_blank" href="'. base_url('integration/user_orders') .'"> Order Details </a> Order ID', $text);
		}
		...
		$text = str_replace('ip_message', "", $text);
		return array($text,$ips);
