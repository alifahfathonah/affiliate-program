...
class Report_model extends MY_Model{
	...
	public function getAllTransaction($filter = array()){
		...
		foreach ($results as $key => $value) {
			$data[] = array(
				...
				'comment'                  => parseMessage($value['comment'],$value,'usercontrol'),
	...
	public function combine_window($data, $notify = false){
				...
				$json[] = array(
					...
					'title' => '<div class="row">
				                    <div class="col-1 bell-icon">
				                        <i class="fa fa-bell '. (($notify == true) ? 'blink-icon' : '') .'"></i>
				                    </div>
				                    <div class="col-3">
				                        <p>'. date('d-m-Y H:i A', strtotime($log['created_at'])) .'</p>
				                    </div>
				                    <div class="col-8">
				                        <p><b>NEW '. $log['click_type'] .'</b> done from '. $log['flag'] .' '. $log['ip'] .' - <small>'. $log['country_code'] .'</small></p>
				                    </div>
				                </div>' ,
				);
			}
			...
			foreach ($data['integration_orders'] as $order) {
				...
				$json[] = array(
					...
					'title' => '<div class="row">
				                    ...
				                    <div class="col-8">
				                        <p><b>New Order '. $order['order_id'] .' ['. $order['total'] . $order['currency'] .'] </b> done From '. $order['country_code'] .' '. $flag .'</p>
				                    </div>
				                </div>' ,
				);
			}
			...
			foreach ($data['newuser'] as $users) {
				...
				$json[] = array(
					...
					'title' => '<div class="row">
				                    ...
				                    <div class="col-8">
				                        <p><b>New Affiliate</b> "'. $users['username'] .'" From '.  $users['sortname'] .' '. $flag . '</p>
				                    </div>
				                </div>' ,
				);
			}
		...
		/*if(isset($live_log['admin_notifications']) && $live_log['admin_notifications'] == "1"){
			foreach ($data['notifications'] as $notification) {
				$json[] = array(
					'type'  => 'notifications',
					'id'    => $notification['id'],
					'date'  => date('Y-m-d H:i:s', strtotime($notification['notification_created_date'])),
					'title' => '<div class="row">
				                    <div class="col-1 bell-icon">
				                        <i class="fa fa-bell '. (($notify == true) ? 'blink-icon' : '') .'"></i>
				                    </div>
				                    <div class="col-3">
				                        <p>'. date('Y-m-d', strtotime($notification['notification_created_date'])) .'</p>
				                    </div>
				                    <div class="col-8">
				                        <p>
				                        <a href="javascript:void(0)" 
				                        onclick="shownofication('. $notification['notification_id'] . ',' . base_url('admincontrol'. $notification['notification_url']) .')"> 
				                        '. $notification['notification_title'] .'</a></p>
				                    </div>
				                </div>' ,
				);
			}
