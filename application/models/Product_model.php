...
class Product_model extends MY_Model{
    function create_data($product, $details){
        $this->db->insert($product, $details);
    ...
    public function ping($user_id){
        $this->db->query("UPDATE users SET last_ping = '". date("Y-m-d H:i:s") ."' WHERE id = {$user_id} ");
    }
    ...
    public function userdetails($guard = 'administrator'){ return $this->session->userdata($guard); }
    ...
    function getAllUsersExport($filter = array()){
        $query = 'SELECT
            countries.sortname,
            users.*,
            pd.*,
    ...
    function getPopulerUsers($filter = array()){
        ...
        $query = 'SELECT
            ...
            WHERE
                users.TYPE = "user" '. $where .'
            ORDER BY amount DESC ,all_commition DESC
        '. $limit;
    ...
    public function calcCommitions($product, $type= 'sale'){
    ...
    public function formcalcCommitions($product, $type= 'sale'){
        $product = (array)$product;
        $product_price = ((int)$product['quantity'] * (float)$product['product_price']);
        if($type == 'sale'){
            $commission = 0;
