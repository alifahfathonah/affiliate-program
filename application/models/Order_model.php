...
class Order_model extends MY_Model{
    ...
    public function changeStatus($order_id, $status , $comment = ''){
        ...
        if($status == 1){
            $check = $this->db->query("SELECT * FROM  `wallet` WHERE status = 0 AND type IN('sale_commission','refer_sale_commission') AND reference_id = {$order_id} ")->num_rows();
            if($check){
                $this->db->query("UPDATE `wallet` SET status =1 WHERE type IN('sale_commission','refer_sale_commission') AND reference_id = {$order_id} ");
    ...
    public function getAllClickLogs($filter = array()){
        ...
        $union = "SELECT SQL_CALC_FOUND_ROWS * FROM ( ({$union_query1}) UNION ALL ({$union_query2}) UNION ALL ({$union_query3}) ) as tmp";
    ...
    public function getAllOrders($filter = array(), $addShipping = true){
        ...
        $union = "SELECT SQL_CALC_FOUND_ROWS * FROM (({$union_query1}) UNION ALL ({$union_query2})) as tmp";
        $union.= " ORDER BY created_at DESC ";
        if (isset($filter['page'],$filter['limit'])) {
            $offset = (($filter['page']-1) * $filter['limit']);
            $union.= " LIMIT {$offset},". $filter['limit'];
        ...
        foreach ($orders as $key => $value) {
            if($value['type'] == 'store'){
                $products        = $this->Order_model->getProducts($value['id'],['refer_id' => $filter['user_id']]);
    ...
    public function getProducts($order_id,$filter = array()){
        $where = '';
        if(isset($filter['refer_id'])) { $where .= " AND op.refer_id =  ". $filter['refer_id']; }
    ...
    public function orderdelete($order_id, $transaction){
        if($transaction > 0){
            $this->db->query("DELETE FROM wallet WHERE reference_id = {$order_id} AND type IN ('sale_commission') ");
        }
        /*if ($walletCheck) {
