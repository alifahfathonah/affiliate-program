...
class Wallet_model extends MY_Model{
	...
	public function getDeleteData($transaction_id){
			...
			switch ($tran->type) {
				...
				case 'sale_commission':
					if($tran->comm_from == 'ex'){
						...
						$data['removed'][] = array(
							'message' => "Sale Done By ". $tran->name ."<br> <b>IP:</b> ". str_replace("'", "", implode(",", $ips)) ."  - <b>Date:</b> ". $tran->created_at,
	...
	public function getTransaction($filter = array()){
		$select = array(
			...
			"(SELECT payment_method FROM `order` WHERE wallet.reference_id = `order`.id AND wallet.type = 'sale_commission' AND wallet.dis_type is NULL ) as payment_method ",
			...
			"(SELECT total FROM `order` WHERE id=wallet.reference_id AND wallet.type = 'sale_commission') as local_orders_total "
		);
		...
		if (isset($filter['types'])) {
			switch ($filter['types']) {
				...
				case 'sale':
					$where .= ' AND wallet.type IN("sale_commission","sale_commission") ';
					break;
	...
	public function getTotals($filter = array(),$extraTotals = false){
		...
		$data['total_sale_commi'] = (float)$this->db->query('SELECT sum(amount) as total FROM wallet WHERE status > 0 AND  type = "sale_commission" AND '. $where )->row_array()['total'];
		...
		$data['total_no_form_click'] = (float)$this->db->query('SELECT COUNT(action_id) as total FROM form_action WHERE '.$where)->row_array()['total'];
		...
		if($extraTotals){
			$data['affiliates_program'] = (int)$this->db->query('SELECT COUNT(*) as total FROM affiliateads')->row_array()['total'];
			...
			$data['total_sale'] = (int)$this->db->query('SELECT SUM(op.total) as total FROM `order_products` op LEFT JOIN `order` as o ON o.id = op.order_id WHERE o.status = 1 AND '. $where1)->row_array()['total'];


			$data['total_sale_week'] = (float)$this->db->query('SELECT SUM(op.total) as total FROM `order` o LEFT JOIN order_products op ON op.order_id = o.id WHERE '. $where1 .' AND YEARWEEK(o.`created_at`, 1) = YEARWEEK(CURDATE(), 1) AND o.status  = 1')->row_array()['total'];
			$data['total_sale_month'] = (float)$this->db->query('SELECT SUM(op.total) as total FROM `order` o LEFT JOIN order_products op ON op.order_id = o.id WHERE '. $where1 .' AND MONTH(o.`created_at`) = MONTH(NOW())  AND o.status  = 1')->row_array()['total'];
			
			$data['total_sale_year'] = (float)$this->db->query('SELECT SUM(op.total) as total FROM `order` o LEFT JOIN order_products op ON op.order_id = o.id WHERE '. $where1 .' AND YEAR(o.`created_at`) = '.date("Y").' AND o.status  = 1')->row_array()['total'];
			...
			$integration_balance = $this->db->query('SELECT base_url,sum(total) as total FROM integration_orders WHERE  '. $where ." GROUP BY base_url")->result();
			//status > 0 AND
			$integration_balance_week = $this->db->query("SELECT sum(total) as total FROM integration_orders WHERE ". $where ." AND  YEARWEEK(`created_at`, 1) = YEARWEEK(CURDATE(), 1)")->row_array()['total'];
			//status > 0 AND
				
			$integration_balance_month = $this->db->query("SELECT sum(total) as total FROM integration_orders WHERE ". $where ." AND  MONTH(`created_at`) = MONTH(NOW())")->row_array()['total'];
			//status > 0 AND
				
			$integration_balance_year = $this->db->query('SELECT sum(total) as total FROM integration_orders WHERE '. $where .' AND YEAR(created_at) = '.date("Y"))->row_array()['total'];
			// status > 0 AND
			...
			$integration_total_orders = $this->db->query('SELECT base_url,count(*) as total,SUM(total) as total_amount FROM `integration_orders` WHERE  '. $where .' GROUP BY base_url' )->result();
			//status > 0 AND
			...
			$data['weekly_balance'] = $data['total_sale_week'] + $integration_balance_week;
			$data['monthly_balance'] = $data['total_sale_month'] + $integration_balance_month;
			$data['yearly_balance'] = $data['total_sale_year'] + $integration_balance_year;
		}
	...
	public function addTransactionRecursion($data){	
        ...
        $json['label'] = "<span class='badge badge-default p-2'>". cycle_details((int)$total_recurring->total, $next_transaction, $data['endtime'],(float)$total_recurring->total_amount) ."</span>";
