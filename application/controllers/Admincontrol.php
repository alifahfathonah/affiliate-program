...
public function clear_tables() {
	...
	public function update_language(){
		...
		if(!isset($json['errors'])){
			...
			if($language_id == 0){
				$this->db->query("INSERT INTO language SET status='". $status ."', name=". $this->db->escape($name) );
				$language_id = $this->db->insert_id();
			} else {
				$this->db->query("UPDATE language SET status='". $status ."', name=". $this->db->escape($name) ." WHERE id =". $language_id );
			}
	...
	public function mails_edit($template_id){
		...
		if (isset($post['send_test'])) {
			...
			$this->db->query("UPDATE mail_templates SET
				...
				`text` = ". $this->db->escape($this->input->post("text",true)) .",
				`admin_subject` = ". $this->db->escape($this->input->post("admin_subject",true)) .",
				`admin_text` = ". $this->db->escape($this->input->post("admin_text",true)) .",
				`client_subject` = ". $this->db->escape($this->input->post("client_subject",true)) .",
				`client_text` = ". $this->db->escape($this->input->post("client_text",true)) ."
			 WHERE id = ". $post['id']);
	...
	public function mywallet(){
		...
		$this->load->view('admincontrol/includes/sidebar', $data);
		$this->load->view('admincontrol/includes/topnav', $data);
		$this->load->view('admincontrol/users/mywallet', $data);
		$this->load->view('admincontrol/includes/footer', $data);
	}
	...
	public function ajax_dashboard(){
		...
		$commission_filters = array();

		if(empty($userdetails)) redirect('admin');	
		$this->Product_model->ping($userdetails['id']);

		if($post['total_commision_filter_year'] && $post['total_commision_filter_month']){
			$commission_filters = array(
				'total_commision_filter_month' => $post['total_commision_filter_month'],
				'total_commision_filter_year' => $post['total_commision_filter_year'],
			);
		}

		$totals = $this->Wallet_model->getTotals($commission_filters, true);
		$user_count = $this->Product_model->getAllUserrecordCount();
		...
		$data['totals'] = array(
			//'total_balance'              => c_format($totals['total_balance']),
			'total_balance'              => c_format($totals['total_sale_amount']),
			'all_clicks_comm'            => $totals['all_clicks']."/".c_format($totals['all_clicks_comm']),
			'action_count_action_amount' => (int)$totals['integration']['action_count'] .'/'. c_format($totals['integration']['action_amount']),
			'user_count'                 => $user_count,
			'hold_action_count'          => $totals['integration']['hold_action_count'],
			'hold_orders'                => $totals['integration']['hold_orders'],
			'weekly_balance'             => c_format($totals['weekly_balance']),
			'monthly_balance'            => c_format($totals['monthly_balance']),
			'yearly_balance'             => c_format($totals['yearly_balance']),
			
			'_t_paid_comm'               => c_format($_t_paid_comm),
			'_t_unpaid_comm'             => c_format($_t_unpaid_comm),
			'_t_request_comm'            => c_format($_t_request_comm),
			'_t_total_commission'        => c_format($_t_total_commission),
			
			'uf_t_paid_comm'             => $_t_paid_comm,
			'uf_t_unpaid_comm'           => $_t_unpaid_comm,
			'uf_t_request_comm'          => $_t_request_comm,
		);

		$totals = $this->Wallet_model->getTotals(array(), true);
		
		//$data['totals']['full_total_balance']              = c_format($totals['total_balance']);
		$data['totals']['full_total_balance']              = c_format($totals['total_sale_amount']);
		...
		$data['totals']['external_inte_order'] =$this->db->query('SELECT COUNT(id) as total FROM `integration_orders` ')->row()->total;
		$data['totals']['full_digital_orders'] =$this->db->query('SELECT COUNT(op.id) as total FROM `order_products` op LEFT JOIN `order` as o ON o.id = op.order_id WHERE o.status > 0 ')->row()->total;

		$post = $this->input->post(null,true);

		if (isset($post['selectedyear'])) {
			$data['chart'] = $this->Order_model->getSaleChart(array('selectedyear' => $post['selectedyear']),$post['renderChart']);
		}else{
			$data['chart'] = $this->Order_model->getSaleChart(array(),$post['renderChart']);
		}
		...
		$data['live_window'] = $this->Report_model->combine_window($data, true);

		$live_dashboard 	= $this->Product_model->getSettings('live_dashboard');
		$data['sound_status'] = (isset($live_dashboard['admin_sound_status']) && $live_dashboard['admin_sound_status'] == "1") ? 1 : 0; 
		$data['live_dashboard'] = $live_dashboard; 
		...
		$data['integration_data'] = $this->get_integartion_data(true);
		//$data['integration_chart_data'] = $this->get_integration_chart_data(true);
	...
	public function dashboard(){
		...
		$data['title']           = __('admin.admin_dashboard');
		...
		$data['totals'] = $this->Wallet_model->getTotals(
			array(
				//'date' => $filter_date,
		...
		$this->load->view('admincontrol/includes/sidebar', $data);
		$this->load->view('admincontrol/includes/topnav', $data);
		$this->load->view('admincontrol/dashboard/index', $data);
		$this->load->view('admincontrol/includes/footer', $data);
	}
	...
	public function listproduct(){
		...
		$forms = $this->Form_model->getForms();		
 		foreach ($forms as $key => $value) {
 			...
 			$forms[$key]['count_coupon'] = $this->Form_model->getFormCouponCount($value['form_id']);
 			if($value['coupon']){
		...
		$this->load->view('admincontrol/includes/sidebar', $data);
		$this->load->view('admincontrol/includes/topnav', $data);
		$this->load->view('admincontrol/product/index', $data);
		$this->load->view('admincontrol/includes/footer', $data);
	}
	...
	public function listorders(){
		...
		$this->load->view('admincontrol/includes/header', $data);
		$this->load->view('admincontrol/includes/sidebar', $data);$this->load->view('admincontrol/includes/topnav', $data);
		$this->load->view('admincontrol/product/orders', $data);
		$this->load->view('admincontrol/includes/footer', $data);
	}
	...
	function orderaction($order_id, $order_action, $transaction = false){
		$userdetails = $this->userdetails();
		if(empty($userdetails)){
			redirect('admin');
		}
	...
	public function downline($user_id){
		...
		$data['user'] 	= $this->Product_model->getUserDetailsObject($user_id);
		//$data['refer_total'] = $this->Product_model->getReferalTotals($user_id);
		$mylevel = array();
	...
	public function userslist(){
        ...
        /*load tree code*/
		//$data['userslist'] = $this->Product_model->getAllUsersTreeV3(array(),0,true,true);
		/*load tree code*/
		...
		$this->load->view('admincontrol/includes/sidebar', $data);
		$this->load->view('admincontrol/includes/topnav', $data);
		$this->load->view('admincontrol/users/index', $data);
		$this->load->view('admincontrol/includes/footer', $data);
	}
	...
	public function userslisttree(){
		...
		//$data['userslist'] = $this->Product_model->getAllUsersTree();
		//$data['userslist'] = $this->Product_model->getAllUsersTreeV2();
		//$data['userslist'] = $this->Product_model->getAllUsersTreeV3(array(),0,true,true);
	...
	public function store_setting(){
		...
		$commonSetting = array('formsetting','productsetting','store','shipping_setting');
		$post = $this->input->post(null,true);
	...
	/*public function deleteads($adsId = null){
		if(!empty($adsId)) {
			$this->Product_model->getDeleteById($adsId);
			$this->session->set_flashdata('success', __('admin.delete_successfully'));
			redirect('admincontrol/affiliateadslist');
		}
		redirect('admincontrol/affiliateadslist');
	}*/
	...
	function delete_image($image_id = null){
		$userdetails = $this->userdetails();
		$post = $this->input->post(null,true);
	...
	function updatenotify($country_id = null) {
		$userdetails = $this->userdetails();
		$json = array();
		$post = $this->input->post(null,true);
		...
		if(empty($userdetails)){ redirect('admin'); }
			...
			if(!empty($post['id'])){
				...
				if($noti->notification_type == 'integration_orders'){
	...
	public function form_manage($form_id = 0){
		...
		$data['product'] = $this->db->query("SELECT product_id,product_name,product_price,product_type,allow_shipping FROM product")->result_array();		
	...
	public function save_form(){
		...
		if($this->form_validation->run() == FALSE) {
			...
			if(empty($json['errors'])){
				$form = array(
					...
					'sale_commision_type'   => $data['form_commision_type'],
					'sale_commision_value'  => $data['form_commision_value'],
					'click_commision_type'  => $data['form_click_commision_type'],
					'click_commision_ppc'   => $data['form_click_commision_ppc'],
					'click_commision_per'   => $data['form_click_commision_per'],
		...
		if($seo_duplicate)
			$json['errors']['seo'] = 'Seo already exists';
    ...
    public function store_markettools(){
		...
		$this->load->view('admincontrol/includes/sidebar', $data);
		$this->load->view('admincontrol/includes/topnav', $data);
		$this->load->view('admincontrol/store/markettools', $data);
		$this->load->view('admincontrol/includes/footer', $data);
	}
	...
	public function info_remove_order(){
		...
		$data = $this->Order_model->getDeleteData((int)$id, $type);

		$html = '<h6 class="text-center"> Order Amount : '. c_format($data['order_amount']) .' </h6>';
		$html .= '<h6 class="text-center"> Commission Amount : '. c_format($data['commission']) .' </h6><hr>';
		$html .= '<p class="text-center"> Order ID : '. $this->input->post("id",true) .' </p>';
		$html .= '<p class="text-center"> <label>
	...
	public function confirm_remove_order(){
		...
		if($sale_commission){
	        $this->db->query("DELETE FROM wallet WHERE type = 'sale_commission' AND reference_id_2 = {$id}");
	        $this->db->query("DELETE FROM wallet WHERE type = 'refer_sale_commission' AND comm_from = 'ex' AND reference_id_2 = {$id}");
		}
