...
class Store extends MY_Controller {
	...
	public function index($user_id = 0){
		...
		$data['products'] = $this->db->query("SELECT * FROM product WHERE product_status=1 AND on_store = 1")->result_array();
	...
	public function category($category_slug = ''){
		...
		$sql = "SELECT DISTINCT p.* FROM product p LEFT JOIN product_categories pc ON pc.product_id = p.product_id WHERE p.product_status=1 AND p.on_store = 1";
	...
	public function product($affiliate_id = 0, $product_slug){
		...
		if($product_slug){
			$data['product'] = $this->db->query("SELECT * FROM product WHERE  on_store = 1 AND product_slug like '". $product_slug ."' ")->row_array();
			if(!$data['product']){
				show_404();
			}
			else
